		...
		int width, int height)
{
	...
	img->shm.shmid = shmget(IPC_PRIVATE, img->image->height
			* img->image->bytes_per_line, IPC_CREAT | 0777);
	if (img->shm.shmid < 0)
	{
	...
	img->pix = XCreatePixmap(ctx->display, ctx->root, width, height,
			ctx->depth);
	if (img->pix == 0)
	{
		...
		int width, int height)
{
	...
	memset(img->image->data, 0, img->image->height
		* img->image->bytes_per_line);
