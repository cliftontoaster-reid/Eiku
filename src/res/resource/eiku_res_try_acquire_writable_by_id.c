...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   eiku_res_try_acquire_writable_by_id.c             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
...
eiku_res_try_acquire_writable_by_id(t_res_manager* manager, eiku_res_id_t id) {
  ...
  // Use the existing try_acquire_writable function
  t_res* acquired = eiku_res_try_acquire_writable(manager, res);
  if (!acquired) {
    return EIKU_INVALID_RES_ID;  // Acquire failed (resource is shared)
  }

  return acquired->id;  // Return the resource ID
}