...
Test(image_functions, test_eiku_new_image_success) {
  ...
  cr_assert_eq(image->width, width, "Image width should match requested width");
...
Test(image_functions, test_eiku_new_image_various_sizes) {
  ...
  cr_assert_eq(large_image->height, 1080, "Large image height should be 1080");
  // Clean up
...
Test(image_functions, test_eiku_get_data_addr) {
  ...
  cr_assert_not_null(image, "Image creation should succeed for data addr test");
  ...
  cr_assert_eq(data_addr, image->data, "Data address should match image data");
...
Test(image_functions, test_eiku_put_image_to_window_success) {
  ...
  cr_assert_eq(image->type, EIKU_TYPE_XIMAGE, "Should use non-SHM image type");
...
Test(image_functions, test_eiku_put_image_to_window_invalid) {
  ...
  cr_assert_eq(
      result1, EIKU_ERROR_INVALID_PARAM,
...
Test(image_functions, test_image_operations_integration) {
  ...
  if (bpp == 32) {
    ...
    cr_assert_eq(pixels[0], 0xFFFFFF, "Top-left should be white (0+0 is even)");
