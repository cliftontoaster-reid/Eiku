...
Test(core_init, test_eiku_context_stability)
{
	...
	cr_assert_eq(ctx->screen, original_screen,
		"Screen should remain unchanged");
	cr_assert_eq(ctx->root, original_root,
		"Root window should remain unchanged");
	// Flush should work
	XFlush(ctx->display);
#endif
	// Clean up
...
Test(core_init, test_eiku_init_destroy_stress)
{
	...
	for (int i = 0; i < cycles; i++)
	{
		...
		cr_assert_not_null(ctx->display, "Context %d display should be valid",
			i);
