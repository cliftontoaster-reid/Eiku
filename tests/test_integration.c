...
Test(integration, test_multiple_windows_with_titles)
{
	...
	cr_assert_eq(ctx->win_list, window3,
		"Third window should be first in list");
	...
	cr_assert_eq(ctx->win_list, window3,
		"Window list order should be preserved");
#ifdef EIKU_PLATFORM_LINUX
...
Test(integration, test_window_stress)
{
	...
	cr_assert_eq(count, num_windows,
		"All windows should be in the context list");
...
Test(integration, test_context_reuse)
{
	...
	cr_assert_eq(ctx->win_list, window2,
		"Second window should be first in list");
	cr_assert_eq(window2->next, window1, "Second window should point to first");
	// Modify both windows
...
Test(integration, test_window_properties_consistency)
{
	...
	t_eiku_window *window = eiku_new_window(ctx, original_width,
			original_height, original_title);
	...
	for (int i = 0; i < num_title_changes; i++)
	{
		...
		cr_assert_neq(window->window, 0,
			"X11 window should remain valid after title change %d", i + 1);
		cr_assert_not_null(window->gc,
			"Graphics context should remain valid after title change %d", i
			+ 1);
#endif
