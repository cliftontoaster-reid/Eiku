...
Test(window_creation, test_eiku_new_window_multiple) {
  ...
  cr_assert_eq(ctx->win_list, window2, "Newest window should be first in list");
...
Test(window_creation, test_eiku_window_set_title_multiple) {
  ...
  const char *long_title =
      "This is a very long window title that might test memory allocation and "
      "string handling capabilities of the window title setting function";
  bool result3;
...
Test(window_creation, test_eiku_new_window_title_variations) {
  ...
  const char *special_title =
      "Window with Special Characters: àáâãäåæçèéêë!@#$%^&*()";
  t_eiku_window *window2;
...
Test(window_creation, test_eiku_new_window_extreme_dimensions) {
  ...
  t_eiku_window *large_window =
      eiku_new_window(ctx, 3840, 2160, "Large Window");
